...
func (s *InMemoryIdempotencyStore) Find(key string) (ReviewResult, bool) {
	result, ok := s.results[key]
	return result, ok
}

func (s *InMemoryIdempotencyStore) Save(key string, result ReviewResult) {
...
func (s ReviewService) Review(request *domainreturns.ReturnRequest, decision domainreturns.ReviewDecision, reviewer, processor, key string) (ReviewResult, error) {
	if key == "" {
		return ReviewResult{}, ErrIdempotencyKeyRequired
	}
	if result, ok := s.store.Find(key); ok {
		return result, nil
	}
	...
	result := ReviewResult{ReturnRequestID: request.ID(), Status: request.Status(), ReviewedBy: request.ReviewedBy(), ProcessedBy: request.ProcessedBy()}
	s.store.Save(key, result)
	return result, nil
}
