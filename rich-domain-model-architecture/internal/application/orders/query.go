...
func (reader *InMemoryReader) Save(order ordering.Order) error {
	...
	reader.orders[details.OrderID] = details
	return nil
}

func (reader *InMemoryReader) GetOrder(id string) (OrderDetails, error) {
	details, ok := reader.orders[id]
	if !ok {
...
func (reader *InMemoryReader) ListOrders(status ordering.OrderStatus) []OrderSummary {
	result := make([]OrderSummary, 0, len(reader.orders))
	for _, details := range reader.orders {
