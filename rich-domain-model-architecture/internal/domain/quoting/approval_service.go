...
// QuoteApprovalService evaluates policy without changing the Quote
// aggregate. Workflow transitions belong to the aggregate itself.
...
func (QuoteApprovalService) Evaluate(quote Quote) ApprovalDecision {
	...
	for _, line := range quote.Lines() {
		...
		reasons = append(reasons, ApprovalReason{
			Code:    ApprovalReasonCustomBuild,
			Message: "custom-build lines require approval",
		})
	}
