...
func TestShipmentHandlerGetsAndListsShipments(t *testing.T) {
	...
	getShipment := application.NewGetShipmentUseCase(shipmentRepo)
	listShipments := application.NewListShipmentsUseCase(shipmentRepo)
	handler := NewShipmentHandler(getShipment, listShipments)
	...
	shipment, _ := createShipment.Execute(order.ID)

	getRequest := httptest.NewRequest(http.MethodGet, "/shipments/"+shipment.ID, nil)
	...
	if !strings.Contains(getRecorder.Body.String(), `"status":"Shipped"`) {
		t.Fatalf("expected shipped shipment in body, got %s", getRecorder.Body.String())
	}

	listRequest := httptest.NewRequest(http.MethodGet, "/shipments?orderId="+order.ID, nil)
	...
	if !strings.Contains(body, `"id":"`+shipment.ID+`"`) || !strings.Contains(body, `"orderId":"`+order.ID+`"`) {
		t.Fatalf("expected shipment list response to contain shipment id and order id, got %s", body)
	}
}
